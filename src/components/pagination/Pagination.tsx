...
export function Pagination({
  ...
  const [pageBlocks, setPageBlocks] = useState<number>(
    Math.ceil(
      countDevices
        ? countDevices / (COUNT_PAGES_IN_BLOCK * countObjectsInPage)
        : 0
    )
  );
  ...
  const countPages = countDevices
    ? Math.ceil(countDevices / countObjectsInPage)
    : 0;
  const arrNumPages = [];
  ...
  const pages = arrNumPages.filter((p, i) => {
    return (
      i < currentPageBlock * COUNT_PAGES_IN_BLOCK &&
      i >= (currentPageBlock - 1) * COUNT_PAGES_IN_BLOCK
    );
  });
  ...
  return (
    <div className={"flex justify-center pt-3 text-gray-600"}>
      <div className={"flex h-6"}>
        ...
        {isPandingPage ? (
          <div className={"h-6 text-sky-700 font-semibold"}>loading...</div>
        ) : (
          pages.map((p) => (
            <span

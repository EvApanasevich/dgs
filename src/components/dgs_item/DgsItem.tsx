...
export function DgsItem({ deviceId, deviceName, reservPower, time }: DgsItemPropsType) {

   return (
      <Link href={`/indetail/${deviceId}`} className={`${reservPower === 'ready' ? 'border-lime-500' :
         reservPower === 'in_work' ? 'border-yellow-400' :
            reservPower === 'not_ready' ? 'border-red-500' : 'border-gray-300'} flex flex-col relative w-80 border-2 rounded-xl`}>
            ...
            <div className="flex flex-col pt-5">
               ...
               <div className="px-3 py-2 text-center text-sm">Резервный источник {reservPower === 'in_work' ? 'находится в работе' :
                  reservPower === 'ready' ? 'готов к пуску' :
                     reservPower === 'not_ready' ? 'не готов к пуску' : 'не определён'}
               </div>
            </div>
         ...
         <span className="text-[0.6rem] text-center">Обновлено: {time ? time : 'нет информации'}</span>
      </Link>
   )
}
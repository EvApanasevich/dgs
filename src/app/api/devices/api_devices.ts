import {
  convertDate,
  convertMonth,
} from "@/app/indetail/[id]/sensor/[sensorId]/utils";
...
export const authApi = {
  async authorize(cred: { email: string; password: string; language: string }) {
    ...
    const response = await fetch(
      "http://api.mechatronics.by:8080/api/3/login",
      {
        method: "POST",
        body: formData,
        redirect: "follow",
...
export const devicesApi = {
  async getDevices(token: string | undefined) {
    try {
      let responseDevices = await fetch(
        "http://api.mechatronics.by:8080/api/3/get_devices",
        {
          method: "POST",
          headers: {
  ...
  async getDevice(id: string, token: string | undefined) {
    try {
      let responseDeviceProperties = await fetch(
        "http://api.mechatronics.by:8080/api/3/get_device",
        {
          method: "POST",
  ...
  async getDeviceSensors(id: number, token: string | undefined) {
    try {
      let responseDeviceSensors = await fetch(
        "http://api.mechatronics.by:8080/api/3/get_device_sens",
        {
          method: "POST",
          body: JSON.stringify({ id: id }),
          headers: {
  ...
  async getDeviceSensorValuesForPeriod(
    id: number,
    sensorId: number,
    from: string,
    to: string,
    ...
    try {
      let responseDeviceSensorValuesForPeriod = await fetch(
        "http://api.mechatronics.by:8080/api/3/get_sensors",
        {
          method: "POST",
          body: JSON.stringify({
            id: id,
      ...
      let sensorValuesForPeriod =
        await responseDeviceSensorValuesForPeriod.json();
      ...
      for (let valueDate in sensorValuesForPeriod) {
        ...
        let newDate = `${date.getFullYear()}-${convertMonth(
          date.getMonth()
        )}-${convertDate(date.getDate())} ${convertDate(
          date.getHours()
